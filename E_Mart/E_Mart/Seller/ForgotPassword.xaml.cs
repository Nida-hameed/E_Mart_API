    ...
    public partial class ForgotPassword : ContentPage
    {
        ...
        private async void btnReset_Clicked(object sender, EventArgs e)
        {
            try
            {
                ...
                var check = await api.CallApiGetAsync<SELLER_tbl>("api/SELLER_tbl_API/forgotpassword?email=" + txtEmailReset.Text);
                if (check == null)
                {
                ...
                MailProvider.SendEmail(check.SELLER_EMIAL, "Password Forgot Request", "Dear " + check.SELLER_NAME + "!!Your Account Credentials are :<br/>" + check.SELLER_EMIAL + "<br/>"+ check.SELLER_PASSWORD + " <br/> Regards: E-Mart");
        ...
        private async void btnPassReset_Clicked(object sender, EventArgs e)
        {
            ...
            App.code = code;
            App.passwordreset = check;
            MailProvider.SendEmail(check.SELLER_EMIAL, "Passward Reset Code", "Dear " + check.SELLER_NAME + "!< br />Please verify this code " + code +".<br/> Regards: E-Mart");
            UserDialogs.Instance.HideLoading();
            await Navigation.PushAsync(new Seller.VerifyCode());
        }
    }
}